...
## 2025-11-20 — Правка авто‑пресета ИИ и авторизации

- На странице `AICoachPage` добавлен параметр `preset=novice` и автозапрос "Новичок/Beginner" по языку `src/pages/AICoachPage.tsx:53`.
- В `AICoach` автозапуск теперь ждёт `user`, чтобы убрать ложный тост "Требуется авторизация" `src/components/AICoach.tsx:229-234`.
- Ссылки «Практика с AI‑Coach» ведут на `/ai?preset=novice` `src/pages/Beginner.tsx:129`, `src/pages/Friedrich.tsx:149`.
- Системный промпт дополнен инструкцией для пресета «Новичок» с обязательным объяснением Пиф‑Паф `src/integrations/gemini.ts:18-22`.